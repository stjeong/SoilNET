	...
	)
{
    ...
    _glGetIntegerv( texture_check_size_enum, &max_supported_size );
	...
	if( tex_id )
	{
		...
		_glBindTexture( opengl_texture_type, tex_id );
		...
		if( flags & SOIL_FLAG_MIPMAPS )
		{
            ...
            _glTexParameteri( opengl_texture_type, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
            _glTexParameteri( opengl_texture_type, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR );
			check_for_GL_errors( "GL_TEXTURE_MIN/MAG_FILTER" );
		} else
		{
			/*	instruct OpenGL _NOT_ to use the MIPmaps	*/
            _glTexParameteri( opengl_texture_type, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
            _glTexParameteri( opengl_texture_type, GL_TEXTURE_MIN_FILTER, GL_LINEAR );
			check_for_GL_errors( "GL_TEXTURE_MIN/MAG_FILTER" );
		}
		/*	does the user want clamping, or wrapping?	*/
		if( flags & SOIL_FLAG_TEXTURE_REPEATS )
		{
            _glTexParameteri( opengl_texture_type, GL_TEXTURE_WRAP_S, GL_REPEAT );
            _glTexParameteri( opengl_texture_type, GL_TEXTURE_WRAP_T, GL_REPEAT );
			if( opengl_texture_type == SOIL_TEXTURE_CUBE_MAP )
			{
				/*	SOIL_TEXTURE_WRAP_R is invalid if cubemaps aren't supported	*/
                _glTexParameteri( opengl_texture_type, SOIL_TEXTURE_WRAP_R, GL_REPEAT );
			}
		...
		} else
		{
            ...
            _glTexParameteri( opengl_texture_type, GL_TEXTURE_WRAP_S, clamp_mode );
            _glTexParameteri( opengl_texture_type, GL_TEXTURE_WRAP_T, clamp_mode );
			if( opengl_texture_type == SOIL_TEXTURE_CUBE_MAP )
			{
				/*	SOIL_TEXTURE_WRAP_R is invalid if cubemaps aren't supported	*/
                _glTexParameteri( opengl_texture_type, SOIL_TEXTURE_WRAP_R, clamp_mode );
			}
			check_for_GL_errors( "GL_TEXTURE_WRAP_*" );
		}
		/*	done	*/
	...
	)
{
    ...
    _glReadPixels(x, y, width, height, GL_RGB, GL_UNSIGNED_BYTE, pixel_data);
	...
	for( cf_target = ogl_target_start; cf_target <= ogl_target_end; ++cf_target )
	{
		if( buffer_index + DDS_full_size <= (unsigned int)buffer_length )
		{
			...
			if( uncompressed )
			{
				...
				for( unsigned int idx = 0; idx < DDS_full_size; idx += block_size )
				{
					unsigned char temp = DDS_data[idx];
					DDS_data[idx] = DDS_data[idx +2];
					DDS_data[idx +2] = temp;
				}
                _glTexImage2D(
					cf_target, 0,
...
int query_DXT_capability( void )
{
	...
	if( has_DXT_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		/*	we haven't yet checked for the capability, do so	*/
		if( NULL == strstr(
				(char const*)_glGetString( GL_EXTENSIONS ),
...
typedef enum tagSOILErrorResult {
    SOIL_SUCCESS = 0,
...
SOILErrorResult _lastError = SOIL_SUCCESS;

int SOIL_GetLastError()
{
...
BOOL SOIL_Initialize(wchar_t *openglDLLFileName)
{
    ...
    if (hModule == NULL)
    {
        _lastError = SOIL_ERROR_LOADLIBRARY;
        return FALSE;
    }
